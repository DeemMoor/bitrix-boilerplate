...
check "console: модуль переименован"        grep -q 'acme.engine' "$P/console"
check "console: старый namespace вычищен"   bash -c "! grep -q 'Vendor.Engine\|vendor\.engine' '$P/console'"
check "тесты: старый namespace вычищен"     bash -c "! grep -rq 'Vendor.Engine' '$P/tests'"
